...
const ViewRequests = () => {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        ...
        axios
          .post(`${process.env.REACT_APP_API_URL}/viewRequests`, {
          ...
          .catch((err) =>
            alert(err.response?.data?.error || "Fetching requests failed")
          );
      } catch (err) {
  ...
  function handleAccept(e, buyer_id, product_id) {
    ...
    if (txt === true) {
      ...
      // Accepting the request;
      axios
        .post(`${process.env.REACT_APP_API_URL}/acceptRequest`, {
          buyer_id: buyer_id,
        ...
        .catch((err) =>
          alert(err.response?.data?.error || "Accepting request failed")
        );
      // window.location.reload();
    }
  }
  function handleReject(e, buyer_id, product_id, productname) {
    ...
    if (txt === true) {
      ...
      axios
        .post(`${process.env.REACT_APP_API_URL}/rejectRequest`, {
          buyer_id: buyer_id,
          productId: product_id,
        ...
        .catch((err) =>
          alert(err.response?.data?.error || "Rejecting request failed")
        );
      window.location.reload();
    }
  }
          ...
          {requests.map((request, index) => (
            <div className="product-card1" key={index}>
              ...
              <div className="buyer-info">
                <h2>Requested By:</h2>
                <p>
                  <strong>Name:</strong> {request.buyer_name}
                </p>
                <p>
                  <strong>Email address:</strong> {request.buyer_email}
                </p>
                <p>
                  <strong>Contact Info:</strong> {request.contact_info}
                </p>
                <p>
                ...
                <button
                  ...
                  onClick={() =>
                    (window.location = `/view_product/${request.product_id}`)
                  }
                >
                  View Product
                </button>
                <button
                  className="buy-button1 accept"
                  onClick={(e) =>
                    handleAccept(e, request.buyer_id, request.product_id)
                  }
                >
                  Accept
                </button>
                <button
                  className="buy-button1 reject"
                  onClick={(e) =>
                    handleReject(
                      e,
                      request.buyer_id,
