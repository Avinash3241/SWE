...
const EditProduct = () => {
  ...
  useEffect(() => {
    axios
      ...
      .catch((err) =>
        alert(err.response?.data?.error || "Product fetch failed")
      );
  }, [product_id]);

  useEffect(() => {
    if (product && product.seller_id != sessionStorage.getItem("UserId")) {
  ...
  function handleSubmission(e) {
    ...
    axios
      .post(`${process.env.REACT_APP_API_URL}/updateProduct`, {
        product: product,
      ...
      .catch((err) =>
        alert(err.response?.data?.error || "Product updation failed")
      );
  }
