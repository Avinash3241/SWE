      ...
      .then(result => {
        ...
        .then(result2 => {
            ...
            for(var i=0;i<products.length;i++){
                ...
                if(product.buyer_id == buyer_id){
                    // Skip the notification for the buyer who made the request
                    ...
                    pool.query(query5, values5)
                    .then(result5 => {
                        // res.json({ message: 'Product Deleted'});
                        console.log("Inserted the notification into buyer "+product.buyer_id);

                        var query6 = 'UPDATE purchase_requests SET status = $1 WHERE product_id = $2 AND buyer_id = $3';
                        var values6 = ["accepted",productId, product.buyer_id];

                        pool.query(query6, values6)
                        .then(result6 => {
                            console.log("Accepted the purchase requests record for buyer_id ",product.buyer_id);
                
                        })
                        .catch(err => {
                            console.log(err.message);
                            res.status(500).send('Accepting of request in purchase requests failed');
                        });
